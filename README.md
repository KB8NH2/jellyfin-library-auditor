...
- `xlsx_report.py` - combined Excel workbook generation (`audit_results.xlsx`): one named-table sheet per server (with a `Problems` count column and a `Totals` row) plus a per-server `Series Summary` sheet (one row per TV series, with a `Complete` column) and an optional `diffs` sheet, with conditional formatting
