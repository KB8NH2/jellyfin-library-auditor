"""Shared JavaScript asset writer for static comparison reports."""
...
def write_javascript(path: Path) -> None:
    """Write the shared comparison JavaScript asset."""
    path.write_text(_script(), encoding="utf-8")


def _script() -> str:
    """Return the shared comparison JavaScript."""
    return "\n".join(
        (
            "(function () {",
            ...
            "    if (link.textContent.trim() === root.dataset.navCurrent) {",
