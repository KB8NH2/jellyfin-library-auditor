...
from report_theme import THEME_STORAGE_KEY
...
def _script() -> str:
    ...
    return "\n".join(
        (
            "(function () {",
            f"  const themeStorageKey = '{THEME_STORAGE_KEY}';",
            ...
            "    document.querySelectorAll('table[data-hide-same]').forEach((table) => {",
            ...
            "    row.hidden = !(matchesQuery && keepWhenHidingSame);",
            "  });",
            "}",
            "",
